...
function parsefile($xhtml = true, $indent = true, $numerote = true, $fic = NULL)
{
	...
	foreach($input as $line)
	{
		...
		for ($i = 0; $i < strlen($line); ++$i)
		{
			...
			if ($status == S_MULTILINE_COMMENT && $line[$i] == '*' && $line[$i + 1] == '/')
			{
				$output_line.= $line[$i++];
				$output_line.= $line[$i];
				$output_line.= set_status($status, S_NORMAL);
			...
			if (($status == S_NORMAL) && substr($line, $i, 9) == 'function ')
			{
			...
			if (($status == S_NORMAL) && substr($line, $i, 6) == 'class ')
			{
			...
			if (strpos($word_valid_chars, strtolower($line[$i])) !== FALSE)
			{
		...
		if ($indent)
		{
			if (($isParsing && !($status == S_NORMAL && (substr($line, -8) == "&lt;?php" || substr($line, -5) == "&lt;?") && strlen($line) > 8)) || (!$isParsing && ($status == S_NORMAL && substr($line, -5) == "?&gt;")))
			{
				...
				{
					if (!($status == S_NORMAL && $line[0] == '{')) $output_line = "\t$output_line";
				}
			...
			else
			{
				...
				while (($btpos = strpos($output_line, "&lt;", $btpos)) !== FALSE)
				{
					...
					if ($supp_etpos && $sp_etpos) $etpos = ($supp_etpos > $sp_etpos) ? $sp_etpos : $supp_etpos;
				...
				foreach($tags as $val)
				{
					if ($val < 0) $html_indentation+= $val;
					else $later_indent+= $val;
				}

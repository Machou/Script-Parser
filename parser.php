...
$tags_inc_indent = array(
	"html", "head", "style", "body", "div", "table", "tr", "td", "th",
	"form", "select", "ul", "ol", "map"
	);
...
function ahref($word, $style, $href = NULL) {
	if (!$href) $href = "#$word";
	if ($GLOBALS['args']['xhtml'])
		return "<a class=\"$style\" href=\"$href\">$word</a>";
	else
		return "$word";
}
...
function parsefile ($xhtml = true, $indent = true, $numerote = true, $fic = NULL) {
	...
	foreach($input as $line) {
		...
		for ($i = 0; $i< strlen($line); ++$i) {
			...
			if ($status < S_STRING_GUILL && $line[$i] == '/' && $line[$i+1] == '/') {
				$output_line .= set_status ($status, S_COMMENT);
				$output_line .= $line[$i++];
				$output_line .= $line[$i];
				continue;
				// Début de commentaire multiligne
			} else if ($status < S_STRING_GUILL && $line[$i] == '/' && $line[$i+1] == '*') {
				$output_line .= set_status ($status, S_MULTILINE_COMMENT);
				$output_line .= $line[$i++];
				$output_line .= $line[$i];
				continue;
				// Fin de commentaire
			} else if ($status == S_MULTILINE_COMMENT && $line[$i] == '*' && $line[$i+1] == '/') {
				$output_line .= $line[$i++];
				$output_line .= $line[$i];
				$output_line .= set_status ($status, S_NORMAL);
				continue;
			...
			// Variable
			if (($status < S_STRING_APOS ) && ($line[$i] == '$')) {
				...
				continue;
			} else if (($status == S_NORMAL) && substr($line,$i,9) == 'function ') {
				// Définition de fonction
				$output_line .= set_status ($status, S_INTERNAL);
				$output_line .= 'function ';
				$output_line .= set_status ($status, S_NORMAL);
				$i += 9;
				while ($line[$i] == ' ') $output_line .= $line[$i++];
				$new_func = '';
				$output_line .= set_status ($status, S_FUNCTION);
				while ($line[$i] != '(') {
					$new_func .= $line[$i];
				...
				$output_line .= set_status ($status, S_NORMAL);
			} else if (($status == S_NORMAL) && substr($line,$i,6) == 'class ') {
			...
			// On s'intéresse à notre mot en cours si on est en mode normal
			// Sinon, non :)
			if ($status != S_NORMAL) {
				if ( ($indent) && ($status >= S_STRING_GUILL) && ($line[$i]=='{') ) {
					++$str_begin_count;
				} else if ( ($indent) && ($status >= S_STRING_GUILL) && ($line[$i]=='}') ) {
					++$str_end_count;
				}
				$output_line .= $line[$i];
			} else if (strpos($word_valid_chars, strtolower($line[$i]))!==FALSE) {
				// Si le caractère est un caractère valide pour la création d'un mot
				// On l'ajoute au mot en cours
				$word .= $line[$i];
			} else {
				// On a un mot
				...
				if ($ipos !== FALSE) {
					...
					if ($GLOBALS['args']['showincs'] || ($fic == $GLOBALS['lfuncslinks'][$lfpos])) {
		...
		if ($indent) {
			if (
				($isParsing && !($status == S_NORMAL && (substr($line, -8)=="&lt;?php" || substr($line, -5)=="&lt;?") && strlen($line) > 8)) ||
				(!$isParsing && ($status == S_NORMAL && substr($line, -5)=="?&gt;"))
				) {
				...
				while (($btpos = strpos($output_line, "&lt;", $btpos))!==FALSE) {
					...
					elseif ($supp_etpos) $etpos = $supp_etpos;
					elseif ($sp_etpos) $etpos = $sp_etpos;
					else ($etpos=FALSE);
					if ($etpos!==FALSE) {
		...
		if ($isParsing && $indent) {
			...
			$previous_line_terminated =
				((strlen($line)==0) && $previous_line_terminated) ||
				(($status >= S_COMMENT) && $previous_line_terminated) ||
				(
				$status < S_STRING_GUILL &&
				(
				(
				($last_char == ';') &&
				(substr($line, -5)!="&amp;" ) &&
...
if (isset($_SESSION["includes"])) {
	foreach ($_SESSION["includes"] as $f) {
		if (file_exists("./repository/$f")) {
			...
			if ($args['xhtml']) $str .= "</pre>\n</div>\n";
			if ($args['showincs']) echo "$str\n\n";
		}
	}
}

// on s'occuppe du code
if ($args['xhtml']) echo "<div class=\"thecode\">\n<pre>";
